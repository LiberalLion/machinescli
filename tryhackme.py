...
class TryHackMe:
  def __init__(self):
    ...
    self.useragent = f"Python THM Client/{self.version}"
  ...
  def room_details(self, room, verbose=False):
    ...
    room = self._get_http(endpoint=f"/room/{room}")
    if verbose and room:
      room["tasks"] = self._get_http(endpoint=f"/tasks/{room}")
      room["votes"] = self._get_http(endpoint=f"/get-votes/{room}")
      room["videos"] = self._get_http(endpoint=f"/video/get/{room}")
    return room
